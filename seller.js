        function EditProfileModal({ seller, onClose, onSave }) {
            ...
            const [photoPreview, setPhotoPreview] = React.useState(seller.profilePhoto || '');
        ...
        function AttractCustomersModal({ seller, onClose, onShare }) {
        ...
        function CampaignStatusWidget({ sellerId }) {
        ...
        function SellerProfile({ seller, isOwnProfile, onClose, onWhatsApp, onShowSubscription, onAddProduct, onProductClick, onDeleteProduct, onEditProfile, onShare, onAttractCustomers }) {
            const isTempVerified = seller.tempVerifiedUntil && new Date(seller.tempVerifiedUntil) > new Date();
                ...
                <div className="fixed inset-0 bg-[#1a1a1a] z-50 flex flex-col overflow-hidden">
                    <div className="bg-[#1a1a1a] p-4 border-b border-gray-800 flex justify-between items-center">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                onClick={() => {
                                    const link = `${window.location.origin}${window.location.pathname}?seller=${seller.id}`;
                                    if (navigator.share) {
                                        navigator.share({ title: seller.name, text: `Check out ${seller.name} on SearchPadi!`, url: link });
                                    } else {
                                                    ...
                                                    <div className="absolute top-2 right-2 flex gap-1">
                                                        <button
                                                            onClick={(e) => {
                                                                ...
                                                                if (navigator.share) {
                                                                    navigator.share({ title: product.name, text: `Check out ${product.name} on SearchPadi!`, url: link });
                                                                } else {
                                                                    navigator.clipboard?.writeText(link).then(() => alert('🔗 Product link copied!')).catch(() => {
                                                                        prompt('Copy this link:', link);
                                                                    });
                                                                }
                                                            }}
