        function AddProductModal({ onClose, onAdd, isUploading }) {
        ...
        // Campaign Modal
        function SubscriptionModal({ onClose, currentUser }) {
        ...
        function SomtoPromoteChat({ onClose, currentUser }) {
            ...
            // Free slot state
            const [freeSlotAvailable, setFreeSlotAvailable] = React.useState(false);
            const [sellerAlreadyClaimed, setSellerAlreadyClaimed] = React.useState(false);
            const [slotsLeft, setSlotsLeft] = React.useState(0);
            const [isFreeOrder, setIsFreeOrder] = React.useState(false);
            const [slotCheckDone, setSlotCheckDone] = React.useState(false);
            ...
            React.useEffect(() => { scrollDown(); }, [messages, typing, sheet]);
